...
export class QuestionClosedButtonsNodeWidget extends React.Component {
  constructor(props) {
    super(props);
  ...
  onFileSelectValidator = (event, types) => {
    var file = event.target.files[0];
    if (!file || types.some((type) => file.type.includes(type))) {
      return this.onFileSelect(event);
    } else {
      Toaster({
        title: language.invalidFile,
        type: "error",
        closeButton: true,
      });
      console.log("Invalid file", file.type, types);
    }
  };
      ...
      let attachmentOptions = [
        {
          ...
          uploader: (
            <input
              type="file"
              accept="image/jpeg, image/png, video/*, video/mp4, video/x-m4v"
              id={fileInputId + "-gallery"}
              onChange={(event) => {
                this.onFileSelectValidator(event, [
                  "image/jpeg",
                  "image/png",
                  "video",
                ]);
        ...
        },
        {
          ...
          uploader: (
            <input
              ...
              onChange={(event) => {
                this.onFileSelectValidator(event, ["application", ".docx"]);
              }}
            />
          ),
          label: "-document",
    ...
    return (
      <div className="closed-type">
        <figure className="image is-40x40">
          <img src={iconClosedButtons} />
        </figure>
        <p className="section-title buttons">{language.buttons}</p>
      </div>
    );
  };
  ...
  render() {
    ...
    return (
      <div className="whole-question-container">
        ...
        />
        <div
          className="question-node-v2 node-v2 closed-buttons"
